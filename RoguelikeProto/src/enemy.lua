...
Enemy = Class
{
	...
	init = function( self, position )
		self.maxHP = self.maxHP * (10 ^ Game.Difficulty)
		
		self.outlineColor = { 255, 5, 5, 255 }
	...
	function Enemy:OnDeath( killer )
		...
		if killer
		then
			killer.curXP = killer.curXP + (self.xpValue * self.level * (5 ^ Game.Difficulty))
		end
	end
	...
	function Enemy:PathToTarget()
		...
		local originPos = Game.CurLevel:ToTileCoords( self:GetPosition() )
		local targetPos = Game.CurLevel:ToTileCoords( self.targetPlayer:GetPosition() )
		...
		do
			...
			do
				neighbor = current + d
				roomCheck, tileCheck = Game.CurLevel:GetRoom( neighbor )
	...
	function Enemy:UpdateMovementInput()
		...
		do
			distance = pos:dist( player:GetPosition() )
		...
		if #self.aiPath > 0
		then
			local tilePos = Game.CurLevel:ToTileCoords( pos )
		...
		then
			self.aimVect = (self.targetPlayer:GetPosition() - pos):normalized()
		end
	end
	...
	function Enemy:Draw()
		...
		GFX.rectangle( "fill", position.x - self.radius, position.y + self.radius + 2, (self.curHP / self:GetMaxHP()) * self.radius * 2, 4 )
	end